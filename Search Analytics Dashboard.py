...
# ✅ STEP 1: Initialize filter state (only once)
...
# Get options from ORIGINAL data (not filtered)
brand_opts = get_cached_options(st.session_state.queries_original, 'brand')
dept_opts = get_cached_options(st.session_state.queries_original, 'department')
cat_opts = get_cached_options(st.session_state.queries_original, 'category')
subcat_opts = get_cached_options(st.session_state.queries_original, 'sub_category')
class_opts = get_cached_options(st.session_state.queries_original, 'Class')
...
# Show filter inputs (use stored values as default)
date_range = st.sidebar.date_input(
    "📅 Select Date Range", 
    value=st.session_state.filter_settings['date_range'] or default_dates
)

brand_filter = st.sidebar.multiselect(
    "🏷 Brand(s)", 
    options=brand_opts, 
    default=st.session_state.filter_settings['brand_filter'] or brand_opts
)

dept_filter = st.sidebar.multiselect(
    "🏬 Department(s)", 
    options=dept_opts, 
    default=st.session_state.filter_settings['dept_filter'] or dept_opts
)

cat_filter = st.sidebar.multiselect(
    "📦 Category(ies)", 
    options=cat_opts, 
    default=st.session_state.filter_settings['cat_filter'] or cat_opts
)

subcat_filter = st.sidebar.multiselect(
    "🧴 Sub Category(ies)", 
    options=subcat_opts, 
    default=st.session_state.filter_settings['subcat_filter'] or subcat_opts
)

class_filter = st.sidebar.multiselect(
    "🎯 Class(es)", 
    options=class_opts, 
    default=st.session_state.filter_settings['class_filter'] or class_opts
)

text_filter = st.sidebar.text_input(
    "🔍 Filter queries by text (contains)",
    value=st.session_state.filter_settings['text_filter']
)

# ✅ STEP 4: Filter control buttons
st.sidebar.markdown("---")
...
if reset_filters:
    ...
    # Don't rerun - let the filter application below handle it

# ✅ STEP 6: Apply filters (ALWAYS, on every rerun)
if st.session_state.get('filters_applied', False):
    ...
    # Date filter
    if settings['date_range'] and len(settings['date_range']) == 2:
    ...
    # Brand filter
    if settings['brand_filter'] and len(settings['brand_filter']) < len(brand_opts):
        queries = queries[queries['brand'].astype(str).isin(settings['brand_filter'])]
    
    # Department filter
    if settings['dept_filter'] and len(settings['dept_filter']) < len(dept_opts):
        queries = queries[queries['department'].astype(str).isin(settings['dept_filter'])]
    
    # Category filter
    if settings['cat_filter'] and len(settings['cat_filter']) < len(cat_opts):
        queries = queries[queries['category'].astype(str).isin(settings['cat_filter'])]
    
    # Subcategory filter
    if settings['subcat_filter'] and len(settings['subcat_filter']) < len(subcat_opts):
        queries = queries[queries['sub_category'].astype(str).isin(settings['subcat_filter'])]
    
    # Class filter
    if settings['class_filter'] and len(settings['class_filter']) < len(class_opts):
        queries = queries[queries['Class'].astype(str).isin(settings['class_filter'])]
    
    # Text filter
    if settings['text_filter']:
        queries = queries[queries['normalized_query'].str.contains(
            re.escape(settings['text_filter']), case=False, na=False)]
...
def standardize_column_names(df):
    """Standardize column names to ensure consistency across all variations"""
    df = df.copy()
    
    # ==================== DATE COLUMNS ====================
    if 'Date' not in df.columns:
        date_candidates = ['start_date', 'date', 'START_DATE', 'query_date', 'Date', 'end_date']
    ...
    if 'search' not in df.columns:
        search_candidates = ['normalized_query', 'query', 'search_query', 'Search', 'Query', 'search_term']
    ...
    if 'count' not in df.columns:
        count_candidates = ['Counts', 'volume', 'search_volume', 'Count', 'Volume', 'total_counts', 'search_count']
        for col in count_candidates:
            if col in df.columns:
                df['count'] = df[col]
                break
    
    # Create total_counts alias
    if 'total_counts' not in df.columns and 'count' in df.columns:
        df['total_counts'] = df['count']
    elif 'total_counts' in df.columns and 'count' not in df.columns:
        df['count'] = df['total_counts']
    
    # ==================== CLICKS COLUMNS ====================
    if 'Clicks' not in df.columns:
        click_candidates = ['clicks', 'click', 'Click', 'total_clicks', 'click_count']
        for col in click_candidates:
            if col in df.columns:
                df['Clicks'] = df[col]
                break
    
    # Create total_clicks alias
    if 'total_clicks' not in df.columns and 'Clicks' in df.columns:
        df['total_clicks'] = df['Clicks']
    elif 'total_clicks' in df.columns and 'Clicks' not in df.columns:
        df['Clicks'] = df['total_clicks']
    
    # ==================== CONVERSIONS COLUMNS ====================
    if 'Conversions' not in df.columns:
        conv_candidates = ['conversions', 'conversion', 'Conversion', 'total_conversions', 'conversion_count']
        for col in conv_candidates:
            if col in df.columns:
                df['Conversions'] = df[col]
                break
    
    # Create total_conversions alias
    if 'total_conversions' not in df.columns and 'Conversions' in df.columns:
        df['total_conversions'] = df['Conversions']
    elif 'total_conversions' in df.columns and 'Conversions' not in df.columns:
        df['Conversions'] = df['total_conversions']
    
    # ==================== BRAND COLUMNS ====================
    if 'brand' not in df.columns:
        brand_candidates = ['Brand', 'brand_name', 'Brand_Name', 'brand_type']
    ...
    if 'department' not in df.columns:
        dept_candidates = ['Department', 'dept', 'Dept', 'department_name']
        for col in dept_candidates:
            if col in df.columns:
                df['department'] = df[col]
                break
    
    # ==================== CATEGORY COLUMNS ====================
    if 'category' not in df.columns:
        cat_candidates = ['Category', 'cat', 'Cat', 'category_name']
    ...
    if 'subcategory' not in df.columns:
        subcat_candidates = ['Subcategory', 'sub_category', 'SubCategory', 'subcategory_name']
        for col in subcat_candidates:
            if col in df.columns:
                df['subcategory'] = df[col]
                break
    
    # ==================== CLASS COLUMNS ====================
    if 'class' not in df.columns:
        class_candidates = ['Class', 'product_class', 'ProductClass', 'class_name']
    ...
    if 'query_length' not in df.columns:
        ...
        if 'search' in df.columns:
            search_col = 'search'
        elif 'normalized_query' in df.columns:
            search_col = 'normalized_query'
        elif 'query' in df.columns:
            search_col = 'query'
        
        if search_col and search_col in df.columns:
            df['query_length'] = df[search_col].astype(str).str.len()
    
    # Word Count
    if 'word_count' not in df.columns:
        search_col = 'search' if 'search' in df.columns else ('normalized_query' if 'normalized_query' in df.columns else None)
        if search_col and search_col in df.columns:
            df['word_count'] = df[search_col].astype(str).str.split().str.len()
    
    # CTR (Click-Through Rate)
    if 'ctr' not in df.columns and 'Clicks' in df.columns and 'count' in df.columns:
        df['ctr'] = np.where(df['count'] > 0, (df['Clicks'] / df['count']) * 100, 0)
    
    # Conversion Rate
    if 'conversion_rate' not in df.columns and 'Conversions' in df.columns and 'count' in df.columns:
        df['conversion_rate'] = np.where(df['count'] > 0, (df['Conversions'] / df['count']) * 100, 0)
...
if st.session_state.get('filters_applied', False):
    ...
    st.sidebar.success(f"✅ Filters Active - {current_count:,} rows ({reduction_pct:.1f}% filtered)")
else:
...
with tab_generic:
    ...
    def get_generic_hero_html():
        ...
        border-radius:20px;margin-bottom:2rem;box-shadow:0 8px 32px rgba(25,118,210,0.15);border:1px solid rgba(33,150,243,0.2);">
            ...
            <p style="color:#1565C0;margin:1rem 0 0 0;font-size:1.3rem;font-weight:300;opacity:0.9;">
                Deep dive into generic type performance and search trends (brand='Other')
            </p>
        </div>
        """
